...
커밋 요청이 있으면 `infra` repo 안에서만 커밋하고 푸시합니다. 작업은 ticket branch에서 진행하고 PR을 생성합니다.
...
```text
feat(logging): [V2V-123] 로그 운영 MVP 인프라 추가
...
local hook과 GitHub Actions `git-policy` workflow가 Git identity, branch, 커밋 제목, 본문을 검사합니다. `main` branch는 GitHub ruleset에서 `git-policy` required check와 force push 금지를 적용합니다.
