...
| `k8s/engine-audio-ingest.yaml` | Audio Ingest 엔진 개발 환경용 `Service`, `Deployment`, PVC입니다. FastAPI HTTP 포트 `8080`을 제공하고 NATS JetStream과 로컬 오디오 자산 저장소를 연결합니다. |
...
- `engine-audio-ingest` 이미지는 애플리케이션 저장소에서 `python:3.12-slim`, FFmpeg/ffprobe, `libsndfile1`, FastAPI/Uvicorn, `nats-py`, `librosa`, `soundfile`, `numpy`, `boto3`를 포함해 빌드되어야 합니다.
